...
CoordVec * get_map(double radius , int divCnt)
{
...
// a – сторона куба (квадрата)
{
	...
	{
		CoordVec * big_map = get_map(radius, divCntBig);
		double big_size = big_map->size() * cube_vol;
		CoordVec * small_map = get_map(radius, divCntSmall);
		...
		curr_map = get_map(radius, divCntBig);
		divCnt = divCntBig;
	}
...
//    cudaSafeCall(cudaMemset(d_cells_cnt, 0, sizeof(int)));
    ...
    cudaSafeCall(cudaDeviceSynchronize());
    cout << time_from_start() << ": Done\n";
	cout << "In result fld setted: " << fld_bit_cnt(result_fld, total_ints) << endl;
...
int main(int argc, char ** argv)
{
    ...
    TCalcPlan plan;
	int plan_error = plan.Init(my_argc, my_argv);
	if (plan_error)
	{
		int i;
		scanf("%d", &i);
